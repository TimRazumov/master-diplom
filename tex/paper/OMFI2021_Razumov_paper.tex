...
%Ранее сервис OCR был написан на языке C++, позже он был %переписан на Golang с полным сохранением функционала. %Переписывание сервиса позволило в 5 раз сэкономить количество  %памяти и cpu.
...
%Сперва в предложенной архитектуре был запущен классификатор %логистической регрессии на мешке слов (bow). На графиках он %обозначен как old clf. Затем был осуществлен запуск нового %классификатора --- полносвязная нейронная сеть на основе fast %text. На рис. \ref{fig:05} и \ref{fig:06} отображены %количество блокировок и количество жалоб на старый и новый %классификатор соответственно. Анализируя эти графики можно %увидеть, что новый классификатор блокирует в 1.5 раза больше, %но при этом на его решения жалуются в 2.5 раза меньше.
...
\begin{tikzpicture}
    ...
    \node (image) at (0,0) {
        \includegraphics[width=0.99\textwidth]{images/old_vs_new_ps.jpg}
    };
...
\caption{\label{fig:05} Blocking old and new classifier.}
\end{figure}
...
\begin{tikzpicture}
    ...
    \node (image) at (0,0) {
        \includegraphics[width=0.99\textwidth]{images/old_vs_new_fp.jpg}
    };
